...
func _ready() -> void:
	MyLogger.log("Loading GameService")
	_pathToField = $Field
...
func makeFieldWalls(mapInfo: MapInfo):
	...
	for xId in range(1, mapSizeX + 1):		#horizontal side wall creation
	...
	for yId in range(1, mapSizeY + 1):		#vertical side wall creation
		var wall = wallForCreation.duplicate()
	...
	#Vertical walls
	for yId in range(0, mapSizeY):
		for xId in range(0, mapSizeX - 1):
			if (verticalWalls[yId][xId]):
				var wall = wallForCreation.duplicate()
				wall.set_rotation_degrees(90)
				...
				_pathToField.add_child(wall)
	#Horizontal walls
	for yId in range(0, mapSizeY - 1):
		for xId in range(0, mapSizeX):
			if (horizontalWalls[yId][xId]):
				var wall = wallForCreation.duplicate()
				_pathToField.add_child(wall)
				wall.name = "HorizontalWall" + str(yId) + "_" + str(xId)
...
func createPlayerRecording(playerInfo: PlayerGameInfoDto, actions: Array, tankName: String):
	...
	new_tank.startingPosition = Vector2(float(firstAction[2])/1000, float(firstAction[3])/1000)
	new_tank.position = new_tank.startingPosition
	#StartingRotation
	var rot = rad_to_deg(Vector2((thisGame.field.sizeX+2)*50, (thisGame.field.sizeY+2)*50).angle_to_point(new_tank.startingPosition)) - 90
	new_tank.rotation_degrees = rot
	new_tank.startingRotation = rot
